...
export async function searchAndFetchContent(
    ...
    statusCallback: (status: string) => void): Promise<SearchResult[]> {
    try {
        ...
        for (const query of optimizedQueries) {
            ...
            try {
                ...
                // Process images
                const images = imageData.items?.slice(0, 3).map((item: { link: any; title: any; image: { thumbnailLink: any; contextLink: any; }; }) => ({
                    url: item.link,
                    title: item.title,
